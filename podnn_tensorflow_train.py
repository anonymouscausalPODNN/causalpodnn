...
class UnitModel(layers.Layer):
    ...
    def build(self,input_shape):
        ...
        for i in range(len(self.unit_model)):
            self.layers.append(copy(self.unit_model[i]))
            #self.layers.append((self.unit_model[i]))
...
class ParallelLayer(layers.Layer):
    ...
    def build(self,input_shape):
        ...
        for i in range(self.n_models):
            ...
            self.model_layers.append(UnitModel(self.unit_model))
            #self.model_layers.append(tf.keras.layers.Conv2D(1, 5, input_shape=[im_height,im_width,1],padding='SAME'))
...
class OrthogonalLayer2D(layers.Layer):
    ...
    def __init__(self, layer_type='dense', n_models=2, n_neurons=10, input_dim=3,orthparam=None):
        super(OrthogonalLayer2D, self).__init__()
        self.dense = orthparam[0]
        temp = 1
...
#        self.w = self.w
    ...
    def call(self,x):
      ...
      #  p = np.random.permutation(n_models)
        ...
        for i in range(1,n_models):
            ...
            m1 = tf.multiply(self.dense.weights[0][0][i-1],m1)   #commented on 24-06-21
            #m1 = tf.multiply(tf.cast(self.dense[0][i - 1],tf.float64), m1)
            #print(self.dense.weights[0][0])
...
class AggregationLayer(layers.Layer):
    ...
    def __init__(self,stride=2,output_dim=1):
        ...
        for i in range(0, self.n_models, self.stride):
            if i >= self.n_models - self.stride + 1:
                ...
                break
            self.aggs.append(layers.Dense(self.output_dim,activation='relu'))
    ...
    def call(self,x):
        ...
        for i in range(self.stride,self.n_models,self.stride):
            ...
            aggregated_out_cuurent = tf.keras.activations.sigmoid(self.aggs[counter](aggregated_in))
